...
// XDM: used with permission
// NOTE: updated, not checked
//=======================================================================
...
public:
	...
	FORCEINLINE matrix3x3(const Vector4D &quaternion)
	{
	...
	// init from angles
	FORCEINLINE matrix3x3(const Vector &angles)
	{
	...
	FORCEINLINE int operator == (const matrix3x3 &mat2) const
	{
	...
	FORCEINLINE int operator != (const matrix3x3 &mat2) const { return !(*this == mat2); }
	matrix3x3& operator=(const matrix3x4 &vOther);
	matrix3x3& operator=(const matrix4x4 &vOther);

	// multiply by a vector (same as Matrix3x3_VectorIRotate).
	// FIXME: remove these
	FORCEINLINE Vector	operator*(const Vector &vec) const
	{
		Vector out;
	...
	Vector	GetForward(void) const { return mat[0]; };
	Vector	GetRight(void) const { return mat[1]; };
	Vector	GetUp(void) const { return mat[2]; };
...
public:
	...
	// init from entity
	FORCEINLINE matrix3x4(const Vector &origin, const Vector &angles, float scale = 1.0f)
	{
		float sr, sp, sy, cr, cp, cy;
		if (angles[ROLL])
		{
	...
	FORCEINLINE matrix3x4(const Vector &origin, Vector4D quaternion)
	{
		mat[0][0] = 1.0f - 2.0f * (quaternion.y * quaternion.y + quaternion.z * quaternion.z);
		mat[1][0] = 2.0f * (quaternion.x * quaternion.y - quaternion.z * quaternion.w);
		mat[2][0] = 2.0f * (quaternion.x * quaternion.z + quaternion.y * quaternion.w);
	...
	FORCEINLINE matrix3x4(const matrix3x3 &in)
	{
		mat[0][0] = in[0][0];
		mat[0][1] = in[0][1];
		mat[0][2] = in[0][2];
	...
	FORCEINLINE int operator == ( const matrix3x4& mat2 ) const
	{
		if( mat[0][0] != mat2[0][0] || mat[0][1] != mat2[0][1] || mat[0][2] != mat2[0][2] )
			return false;
		if( mat[1][0] != mat2[1][0] || mat[1][1] != mat2[1][1] || mat[1][2] != mat2[1][2] )
	...
	FORCEINLINE int operator != (const matrix3x4& mat2) const { return !(*this == mat2); }
...
public:
	...
	// init from entity
	FORCEINLINE matrix4x4(const Vector &origin, const Vector &angles, float scale = 1.0f)
	{
		float sr, sp, sy, cr, cp, cy;
		if (angles[ROLL])
		{
	...
	// init from quaternion + origin
	FORCEINLINE matrix4x4(const Vector &origin, Vector4D quaternion)
	{
		mat[0][0] = 1.0f - 2.0f * (quaternion.y * quaternion.y + quaternion.z * quaternion.z);
		mat[1][0] = 2.0f * (quaternion.x * quaternion.y - quaternion.z * quaternion.w);
		mat[2][0] = 2.0f * (quaternion.x * quaternion.z + quaternion.y * quaternion.w);
	...
	// init from matrix3x4
	FORCEINLINE matrix4x4(const matrix3x4 &in)
	{
		mat[0][0] = in[0][0];
		mat[0][1] = in[0][1];
		mat[0][2] = in[0][2];
	...
	FORCEINLINE int operator == (const matrix4x4& mat2) const
	{
	...
	FORCEINLINE int operator != ( const matrix4x4& mat2 ) const { return !(*this == mat2 ); }
