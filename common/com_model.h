...
// XHL: universal types for online selection
/*
typedef void * texture_t;
typedef void * mnode_t;
typedef void * mleaf_t;
typedef void * msurface_t;
*/
#ifdef HARDWARE_MODE
typedef struct texture_hw_s texture_t;
typedef struct mnode_hw_s mnode_t;
typedef struct mleaf_hw_s mleaf_t;
typedef struct msurface_hw_s msurface_t;
#else
typedef struct texture_sw_s texture_t;
typedef struct mnode_sw_s mnode_t;
typedef struct mleaf_sw_s mleaf_t;
typedef struct msurface_sw_s msurface_t;
#endif
...
// common with leaf
	...
	short			minmaxs[6];		// for bounding box culling. SW.
	mnode_t			*parent;
// node specific
	mplane_t		*plane;
	mnode_t			*children[2];
...
//  hardware renderer - QW 'gl_model.h'
//  06/23/2002 2230 MAH
//  WARNING - the above indicates this structure was modified for Half-Life this structure needs VERIFICATION
//  06/23/2002 2300 MAH - the below version for hardware agrees with a hexadecimal data dump of these structures taken from a running game.
typedef struct msurface_hw_s
{
	...
	struct msurface_hw_s	*texturechain;
