	...
	Panel **pParentPanel)
{
	...
	m_pCvarModEnable = CVAR_CREATE("voice_modenable", "1", FCVAR_ARCHIVE | FCVAR_CLIENTDLL);
	m_pCvarClientDebug = CVAR_CREATE("voice_clientdebug", "0", FCVAR_CLIENTDLL);

	gEngfuncs.pfnAddCommand("voice_showbanned", CMD_ShowBanned);
...
void CVoiceStatus::CreateEntities()
{
	...
	for (int i=0; i < VOICE_MAX_PLAYERS; i++)
	{
		...
		if (pClient == localPlayer && !CL_IsThirdPerson())// XDM
			continue;
...
//			gEngfuncs.pfnClientCmd( "voice_modenable 1" );
			gEngfuncs.Cvar_SetValue(m_pCvarModEnable->name, 1.0f);// TESTME
		}
	}
	else if( entindex == -2 )
	{
...
void CVoiceStatus::UpdateServerState(bool bForce)
{
	...
	int bCVarModEnable = (m_pCvarModEnable->value > 0.0f);
...
//	CON_PRINTF("UpdateSpeakerImage(%d)\n", iPlayer);
...
//		CON_DPRINTF("UpdateBanButton(%d) banned\n", iClient);
	}
	else if (bTalking)
	{
		if (bBlink)
...
//		CON_DPRINTF("UpdateBanButton(%d) never spk\n", iClient);
	}
	else
	{
		pPanel->setImage(m_pScoreboardNotSpeaking);
//		CON_DPRINTF("UpdateBanButton(%d) not spk\n", iClient);
	}
}
...
void CVoiceStatus::RepositionLabels()
{
	...
	if( m_pLocalBitmap && m_pAckBitmap && m_pLocalLabel && (m_bTalking || m_bServerAcked) )
	{
		...
		if (m_bServerAcked && (m_pCvarClientDebug->value > 0.0f))
