...
int CHudGeiger::Draw(const float &flTime)
{
	int pct;
	float flvol;
	int rg[3];// wav name (number)
	int i;// number of wav names
	...
	if (m_iGeigerRange < 1000 && m_iGeigerRange > 0)
	{
		...
		if (i>0 && pct>0 && ((rand() & 127) < pct || (rand() & 127) < pct))
		{
			//S_StartDynamicSound (-1, 0, rgsfx[rand() % i], r_origin, flvol, 1.0, 0, 100);	
			char sz[256];
			sprintf(sz, "player/geiger%d.wav", rg[RANDOM_LONG(0,i-1)]);
			PlaySound(sz, flvol);
		}
	}
	return 1;
