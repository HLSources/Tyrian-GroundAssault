...
int CHudMessage::Draw(const float &flTime)
{
	...
	if (m_gameTitleTime > 0)
	{
		...
		else
		{
			...
			int r,g,b;// pfnSPR_Set uses int
			r = m_pGameTitle->r1;
			g = m_pGameTitle->g1;
			b = m_pGameTitle->b1;
			ScaleColors(r,g,b, brightness);
	...
	for (i = 0; i < maxHUDMessages; ++i)
	{
		...
		{
			pMessage = m_pMessages[i];
//			CON_DPRINTF("CHudMessage::Draw(%d '%s' '%s')\n", i, pMessage->pName, pMessage->pMessage);
	...
	for (i = 0; i < maxHUDMessages; ++i)
	{
		if (m_pMessages[i] == NULL)
		{
			...
			else
			{
				...
				{
					CON_DPRINTF("Text message %s has invalid effect %d\n", pName, tempMessage->effect);
...
{
//	CON_DPRINTF("CHudMessage::MessageAdd('%s', '%s')\n", newMessage->pName, newMessage->pMessage);
	m_parms.time = gHUD.m_flTime;
