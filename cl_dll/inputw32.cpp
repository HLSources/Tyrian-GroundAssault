...
//	{
	...
	if (g_iVisibleMouse)// XDM3035a: mouse coords are [-ScreenResPx/2...+ScreenResPx/2]
	{
		mouse_x = mx;
		mouse_y = my;
...
{
	if (gHUD.m_Health.m_iHealth <= 0)// XDM
		return;

	if (/*!g_iVisibleMouse && */mouseactive)
...
void IN_Init(void)
{
	...
	sensitivity				= CVAR_CREATE ( "sensitivity","3", FCVAR_ARCHIVE ); // user mouse sensitivity setting.

	if (gEngfuncs.CheckParm("-nojoy", NULL) == 0)// XDM3037: less vars - faster it runs
	{
