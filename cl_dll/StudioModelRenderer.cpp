...
//const vec3_t vec3_255(255,255,255);
alight_t aLightFullBright;// = { 128, 192, vec3_255, vec3_origin };// XDM3035: replace plightvec with current model origin
...
void CStudioModelRenderer::Init( void )
{
	...
	aLightFullBright.color[0] = 255;
	aLightFullBright.color[1] = 255;
	aLightFullBright.color[2] = 255;
	aLightFullBright.plightvec = (float *)&g_vecZero;

	aLightEntityHighlight.ambientlight = 1;
	aLightEntityHighlight.shadelight = 1;
	aLightEntityHighlight.color[0] = 255;
	aLightEntityHighlight.color[1] = 255;
	aLightEntityHighlight.color[2] = 255;
	aLightEntityHighlight.plightvec = (float *)&g_vecZero;
...
void CStudioModelRenderer::StudioPlayerBlend( mstudioseqdesc_t *pseqdesc, int *pBlend, float *pPitch )
{
	...
	*pBlend = (int)(*pPitch * 3.0f);// XDM3035c
	...
	else
	{
		if (pseqdesc->blendend[0] - pseqdesc->blendstart[0] < 0.1) // catch qc error
			...
			*pBlend = (int)(255.0f * (*pBlend - pseqdesc->blendstart[0]) / (pseqdesc->blendend[0] - pseqdesc->blendstart[0]));
		*pPitch = 0;
	}
}
...
//LODs for projectiles.mdl, w_items.mdl, ef_teleporter.mdl, ef_lightning_field.mdl
	if (g_pCvarEnableLods->value > 0.0f)
	{
		if (UTIL_PointIsFar(m_pCurrentEntity->origin, 0.5))
		{
			if ( strstr( m_pCurrentEntity->model->name, "projectiles" ) || strstr( m_pCurrentEntity->model->name, "w_items" ) || strstr( m_pCurrentEntity->model->name, "ef_teleporter" ))
				m_pCurrentEntity->curstate.body = m_pCurrentEntity->curstate.body + 1;
		}

		if (UTIL_PointIsFar(m_pCurrentEntity->origin, 0.75))
		{
			if ( strstr( m_pCurrentEntity->model->name, "ef_explosions" ) || strstr( m_pCurrentEntity->model->name, "ef_lightning_field" ))
				m_pCurrentEntity->curstate.body = m_pCurrentEntity->curstate.body + 1;
		}
	}
/*	
	//Rotate some models. Should work for RSModel, but does not
	//Yeah, we're too lazy to animate them!.
	if ( strstr( m_pCurrentEntity->model->name, "ef_lightning_field" )  )
	{
		float timemod;
		timemod = fmod( gEngfuncs.GetClientTime(), 0.5 );
...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	if (flags & STUDIO_EVENTS)
	{
		...
		if (m_pCurrentEntity->index > 0)
		{
			...
			memcpy(ent->attachment, m_pCurrentEntity->attachment, sizeof(vec3_t) * 4);// MAXSTUDIOATTACHMENTS
		}
	}
...
//		{
		...
		{
//			aLightEntityHighlight.ambientlight = CVAR_GET_FLOAT("test1");
//			aLightEntityHighlight.shadelight = (int)(g_fEntityHighlightEffect*CVAR_GET_FLOAT("test2"));
...
// Don't animate players, if model IsFar
	if (pplayer->gaitsequence /*&& (UTIL_PointViewDist(m_pCurrentEntity->origin) <= 2.0f)*/)// XDM3035c
	{
		vec3_t orig_angles;
	...
	{
/*		int iBlend = 127;// XDM3035b: why isn't local blending working??
		mstudioseqdesc_t *pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pCurrentEntity->curstate.sequence;
		...
		m_pCurrentEntity->latched.prevseqblending[0] = m_pCurrentEntity->curstate.blending[0];
*/
		m_pCurrentEntity->curstate.blending[0] = 127;// XDM3035b reset?
		m_pCurrentEntity->curstate.blending[1] = 127;
		m_pCurrentEntity->curstate.blending[2] = 127;
		m_pCurrentEntity->curstate.blending[3] = 127;
...
//		m_pPlayerInfo = IEngineStudio.PlayerInfo( m_nPlayerIndex );
	...
	if (flags & STUDIO_RENDER)
	{
		//LODs for player model
		if (g_pCvarEnableLods->value > 0.0f)
		{
			if (m_pCurrentEntity)
			{
				if (UTIL_PointIsFar(m_pCurrentEntity->origin, 1.0))
					m_pCurrentEntity->curstate.body = 2;
				else if (UTIL_PointIsFar(m_pCurrentEntity->origin, 0.5))
		...
		alight_t lighting;
		vec3_t dir;
		lighting.plightvec = dir;// somebody tell me wtf is this?!

//XDM3035: works but do we need it?		if (!UTIL_PointIsVisible(m_pCurrentEntity->origin, true))
//			return 0;
/*
ZOMGWTF!
BUGBUG: this randomly makes entities visible through walls!!

		if (IsTeamGame(gHUD.m_iGameType) && gHUD.m_pCvarHLPlayers->value > 0.0f)// XDM3035: highlight teammates
		{
			vec3_t d;
			VectorSubtract(gHUD.m_LocalPlayerState.origin, m_pCurrentEntity->curstate.origin, d);
			float dist = Length(d);
			if (dist < g_fPlayerHighlightMaxDist)
			{
				float k = 0.5f*(1.0f - dist/g_fPlayerHighlightMaxDist);// 0.5 is the amount of color
				int r,g,b;
				GetTeamColor(g_PlayerExtraInfo[m_nPlayerIndex].teamnumber, r,g,b);
				vec3_t teamcolor;
				teamcolor[0] = ((float)b/255.0f)*k;// WTF? BGR?
				teamcolor[1] = ((float)g/255.0f)*k;
				teamcolor[2] = ((float)r/255.0f)*k;
				// c = a + sb
				VectorMA(teamcolor, 1.0f-k, lighting.color, lighting.color);
				if (g_PlayerExtraInfo[m_nPlayerIndex].teamnumber == gHUD.m_LocalPlayerState.team && m_nPlayerIndex != gEngfuncs.GetLocalPlayer()->index)
				{
					// draw extra pointers above teammates
					vec3_t vec, screen;
					VectorCopy(m_pCurrentEntity->curstate.origin, vec);
					vec[2] += 48.0f;
					if (gEngfuncs.pTriAPI->WorldToScreen(vec, screen) != 1)
					{
						screen[0] = XPROJECT(screen[0]);
						screen[1] = YPROJECT(screen[1]);
						screen[2] = 0.0f;
						// draw the players name
						char *string = g_PlayerInfoList[m_nPlayerIndex].name;
						int lx = strlen(string)*3; // 3 is avg. character length :)
						DrawSetTextColor(r,g,b);
						DrawConsoleString(screen[0]-lx, screen[1], string);
					}
				}
			}
		}
...
//		IEngineStudio.StudioSetRemapColors(m_nTopColor, m_nBottomColor);
		...
		if (pplayer->weaponmodel /*&& !UTIL_PointIsFar(m_pCurrentEntity->origin)*/)// XDM3035c
		{
...
/*			if (viewmodel)
			{
				mstudiotexture_t *pTextureHeader = (mstudiotexture_t *)((byte *)m_pStudioHeader + m_pStudioHeader->textureindex);
				IEngineStudio.StudioSetupSkin(pTextureHeader, viewmodel->curstate.skin);// XDM3035c: Why doesn't this work?!
			}*/
//			IEngineStudio.StudioSetRemapColors(m_nTopColor, m_nBottomColor);// XDM3034: same colors?
			IEngineStudio.StudioSetupLighting(&lighting);
			StudioRenderModel();
//Ghoul: this allows to use _p.mdl attachments 
...
void CStudioModelRenderer::StudioCalcAttachments( void )
{
	if (m_pStudioHeader->numattachments > 4)
	{
		...
		m_pStudioHeader->numattachments = 4;
// XDM		exit( -1 );
	}
...
void CStudioModelRenderer::StudioRenderFinal_Software( void )
{
	...
	if (m_pCvarDrawEntities->value == 6 && gHUD.m_iPaused == 0)
	{
//		mstudioattachment_t *pAttachments = (mstudioattachment_t *)((byte *)m_pStudioHeader + m_pStudioHeader->attachmentindex);
//		mstudiobone_t *pBones = (mstudiobone_t *)((byte *)m_pStudioHeader + m_pStudioHeader->boneindex);
		for (int i = 0; i < m_pStudioHeader->numattachments; ++i)
...
//			gEngfuncs.pTriAPI->Vertex3fv(m_pCurrentEntity->attachment[i]);
			gEngfuncs.pEfxAPI->R_MuzzleFlash((float *)&m_pCurrentEntity->attachment[i], 10+i);
		}
	}
