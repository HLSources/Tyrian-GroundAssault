...
void CMD_SpectatorMode(void)
{
	if (CMD_ARGC() <= 1)
	{
		CON_PRINTF("usage:  %s <main mode> [inset mode]\n", CMD_ARGV(0));
		return;
	}
...
void CMD_SpectatorHelp(void)
{
	if (gViewPort)
	{
		gViewPort->ShowMenu(MENU_SPECHELP);
	}
	else
	{
...
void CMD_SpectatorPIP(void)// XDM
{
	if (CMD_ARGC() <= 1)
	{
		CON_PRINTF("usage: %s <inset mode>  current mode: %d\n", CMD_ARGV(0), gHUD.m_Spectator.m_iInsetMode);
		return;
	}
...
//	m_pip			= CVAR_CREATE("spec_pip","3",FCVAR_CLIENTDLL);
	...
	{
		CON_PRINTF("ERROR! Couldn't register all spectator variables.\n");
		return 0;
	}
...
void CHudSpectator::SetSpectatorStartPosition(void)
{
	...
	else
	{
		...
		VectorCopy(g_vecZero, m_cameraOrigin);
		VectorCopy(g_vecZero, m_cameraAngles);
	}
...
{
	m_hsprPlayer		= SPR_Load("sprites/spec_player.spr");
	m_hsprPlayerDead	= SPR_Load("sprites/spec_playerdead.spr");
	m_hsprUnkownMap		= SPR_Load("sprites/spec_tile.spr");
	m_hsprBeam			= SPR_Load("sprites/laserbeam.spr");
	m_hsprCamera		= SPR_Load("sprites/spec_camera.spr");
	m_hCrosshair		= SPR_Load("sprites/spec_crosshair.spr");
	return 1;
}
...
int CHudSpectator::Draw(const float &flTime)
{
	...
	if (g_iUser1 < OBS_MAP_FREE  || m_drawnames->value <= 0.0f) 
		return 1;
...
//							g_iPlayerClass = 0;
		...
		case DRC_CMD_EVENT:
							...
							// CON_PRINTF("Director Camera: %i %i\n", firstObject, secondObject);
							break;
		...
		case DRC_CMD_SOUND:
							string = READ_STRING();
							value =  READ_FLOAT();
							// CON_PRINTF("DRC_CMD_FX_SOUND: %s %.2f\n", string, value );
		...
		default: CON_DPRINTF("CHudSpectator::DirectorMessage: unknown command %i.\n", cmd ); break;
	}
//	END_READ();
}
...
void CHudSpectator::FindNextPlayer(bool bReverse)
{
	...
	do
	{
		...
		if (IsTeamGame(gHUD.m_iGameType))
		{
			...
			if (gHUD.m_LocalPlayerState.playerclass != TEAM_NONE && pEnt->curstate.team != gHUD.m_LocalPlayerState.playerclass)
				continue;
		}
	...
	if (!g_iUser2)
	{
		CON_DPRINTF("No observer targets.\n");
...
//	CON_PRINTF("SetModes(%d %d)\n", iNewMainMode, iNewInsetMode);
	...
	{
		CON_PRINTF("Invalid spectator mode.\n");
		return;
	}
	...
	if (iNewMainMode != g_iUser1)
	{
		...
		{
			char str[16];
			sprintf(str, "spec_mode %d", iNewMainMode);
			SERVER_COMMAND(str);
			return;
		}
		...
		else if (g_iUser1 == OBS_MAP_FREE || g_iUser1 == OBS_MAP_CHASE)
		{
			// reset user values
...
//			m_crosshairRect.bottom = 24;
			SetCrosshair(m_hCrosshair, m_crosshairRect, 255, 180, 0);
		}
		else
		{
...
bool CHudSpectator::ParseOverviewFile(void)
{
	...
	m_OverviewData.layers = 0;// XDM: set to 1?
	m_OverviewData.layersHeights[0] = 0.0f;
	strncpy(m_OverviewData.map, GET_LEVEL_NAME(), MAX_MAPNAME);// XDM

	if (strlen(m_OverviewData.map) == 0)
		return false; // not active yet

	strcpy(levelname, m_OverviewData.map + 5);// maps/
	...
	if (!pfile)
	{
		CON_PRINTF("Couldn't open file %s. Using default values for overiew mode.\n", filename );
		m_mapZoom = m_OverviewData.zoom;
		m_mapOrigin = m_OverviewData.origin;
		return false;
	}
...
//		else if (!strncmp(token, "/*", 2))
		...
		if (!stricmp(token, "global"))
		{
			...
			while (stricmp(token, "}"))
			{
				...
				else if (!stricmp(token, "inset"))
				{
					...
					CON_PRINTF("Warning! Unknown keyword '%s' in overview file %s\n", token, filename);
		...
		else if (!stricmp(token, "layer"))
		{
			...
			if (m_OverviewData.layers >= OVERVIEW_MAX_LAYERS )
				CON_PRINTF("Warning! Too many layers in overview file %s\n", filename);
...
/*				else if (!stricmp(token, "rendermode"))// UNDONE
				{
...
void CHudSpectator::LoadMapSprites(void)
{
	...
	for (int i = 0; i < OVERVIEW_MAX_LAYERS; ++i)// XDM: multilayer support
	{
		if (i < m_OverviewData.layers/* && m_OverviewData.layersImages[i][0] != NULL*/)
			m_MapSprite[i] = gEngfuncs.LoadMapSprite(m_OverviewData.layersImages[i]);
		else// the standard "unkown map" sprite will be used instead
		{
			if (i == 0)// replace only first layer with grid
				m_MapSprite[i] = m_DefaultSprite;
			else
				m_MapSprite[i] = NULL;
		}
	}
...
void CHudSpectator::DrawOverviewLayer(void)
{
	...
	for (int current_layer = 0; current_layer < m_OverviewData.layers; current_layer++)// XDM: multilayer support
//	for (int current_layer = m_OverviewData.layers-1; current_layer >= 0; current_layer--)
	{
		...
		if (m_OverviewData.layersHeights[current_layer] >= gHUD.m_vecOrigin[2])// XDM3034
		{
			gEngfuncs.pTriAPI->RenderMode(kRenderTransAlpha);
			gEngfuncs.pTriAPI->Color4f(1.0, 1.0, 1.0, 0.25);
//			CON_PRINTF("OVW: lyr %d\n", current_layer);
		}
		else
		{
...
// BUGBUG: makes entities totally invisible if this function is called more that once!
	...
	{
//		if (m_OverviewEntities[i].hSprite == 0)
		...
		//pSpriteModel = (struct model_s *)gEngfuncs.GetSpritePointer(m_OverviewEntities[i].hSprite);
...
//			up = g_vecViewUp;
		...
		if (ent->curstate.messagenum == gHUD.m_LocalPlayerState.messagenum)// XDM3035c
		...
		if (!ent->player)
			gEngfuncs.pTriAPI->Brightness(0.5f + 0.5f*sinf(gEngfuncs.GetClientTime()*8.0f));
...
/*
		gEngfuncs.pTriAPI->Begin(TRI_QUADS);
			...
			gEngfuncs.pTriAPI->Vertex3f(origin[0]-4, origin[1]-4, origin[2]-zScale);
			gEngfuncs.pTriAPI->TexCoord2f(0, 1);
			gEngfuncs.pTriAPI->Vertex3f(origin[0]-4, origin[1]-4, origin[2]+HULL_MIN);//z);
			gEngfuncs.pTriAPI->TexCoord2f(1, 1);
			gEngfuncs.pTriAPI->Vertex3f(origin[0]+4, origin[1]+4, origin[2]+HULL_MIN);//z);
		gEngfuncs.pTriAPI->End();
...
*/
	...
	if (g_iUser1 == OBS_ROAMING || (gHUD.m_pCvarMiniMap->value > 0.0))// XDM
	{
...
// Output : int
//-----------------------------------------------------------------------------
int CHudSpectator::DrawOverview(void)
{
	// draw only in sepctator mode
	if (g_iUser1 <= 0 && gHUD.m_pCvarMiniMap->value <= 0.0)// XDM
		return 0;

	if (g_iUser1 == OBS_INTERMISSION)// XDM3035: revisit this later
		return 0;

	// Only draw the overview if Map Mode is selected for this view
	if (m_iDrawCycle == 0 && (g_iUser1 < OBS_MAP_FREE)) 
		return 0;

	if (m_iDrawCycle == 1)
	{
		//if (g_iUser1 > OBS_NONE && g_iUser1 != OBS_INTERMISSION)
		if (gHUD.IsSpectator())// XDM3035
		{
			if (m_iInsetMode < INSET_MAP_FREE)
				return 0;
		}
		else
		{
			if (gHUD.m_pCvarMiniMap->value <= 0.0)
				return 0;
		}
	}
...
void CHudSpectator::CheckOverviewEntities(void)
{
	...
	for (int i = 0; i< OVERVIEW_MAX_ENTITIES; ++i)
	{
...
//-----------------------------------------------------------------------------
// XDM: -1.0 means show it only this frame;
// Input  : *ent - 
//			icon - 
//			lifetime - 
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CHudSpectator::AddOverviewEntity(struct cl_entity_s *ent, struct model_s *icon, float lifetime)
{
	if (ent == NULL)
		return false;
...
//			hSprite = m_hsprPlayer;// XDM
	...
	return AddOverviewEntityToList(ent, pSprite, (lifetime == 0)?0:(gEngfuncs.GetClientTime() + lifetime));// XDM3035c
}

//-----------------------------------------------------------------------------
// Purpose: AddOverviewEntityToList
// Input  : *ent - 
//			*sprite - 
//			killTime - 
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CHudSpectator::AddOverviewEntityToList(cl_entity_t *ent, const struct model_s *sprite, float killTime)
{
	for (int i = 0; i < OVERVIEW_MAX_ENTITIES; ++i)
	{
		// find empty entity slot
		if (m_OverviewEntities[i].entity == NULL || m_OverviewEntities[i].entity == ent)// XDM
		{
...
//			CON_PRINTF("CL: AddOverviewEntityToList: (%d) added entiy %d\n", i, ent->index);
			return true;
		}
	}
	CON_DPRINTF("AddOverviewEntityToList(%d): error: too many entities! (max %d)\n", ent->index, OVERVIEW_MAX_ENTITIES);
...
void CHudSpectator::DeathMessage(int victim)
{
	...
	if (pl && pl->player)
		AddOverviewEntityToList(pl, gEngfuncs.GetSpritePointer(m_hsprPlayerDead), gEngfuncs.GetClientTime() + 2.0f);
}
...
void CHudSpectator::CheckSettings(void)
{
	...
	if ((g_iUser1 < OBS_MAP_FREE) && (m_iInsetMode < INSET_MAP_FREE) && (m_iInsetMode != INSET_OFF))
		m_iInsetMode = INSET_MAP_FREE;// otherwise both would show in World picures

	if ((g_iUser1 >= OBS_MAP_FREE) && (m_iInsetMode >= INSET_MAP_FREE))
		m_iInsetMode = INSET_CHASE_FREE;// both would show map views
	...
	gViewPort->GetSpectatorPanel()->EnableInsetView(m_iInsetMode != INSET_OFF);

//	CON_PRINTF("CheckSettings: g_iUser1 = %d\n", g_iUser1);
}
...
{
	if (g_iUser1 < OBS_MAP_FREE)// XDM: main mode not fullscreen MAP
	{
		if (newInsetMode == INSET_CHASE_FREE || newInsetMode == INSET_IN_EYE)
		{
			newInsetMode = INSET_MAP_FREE;
		}
		else if (newInsetMode > INSET_MAP_CHASE)
		{
			if (allowOff)
...
void CHudSpectator::InitHUDData(void)
{
	...
	// reset HUD FOV
	gHUD.m_iFOV = gHUD.GetUpdatedDefaultFOV();// XDM
}
/*
//-----------------------------------------------------------------------------
// Purpose: debug
...
bool CHudSpectator::ShouldDrawOverview(void)
{
	...
	if (gHUD.IsSpectator())
	{
		...
		if ((g_iUser1 >= OBS_MAP_FREE) && (g_iUser1 <= OBS_MAP_CHASE))// XDM3035a
			return true;
	}
	else
	{
		if (gHUD.m_pCvarMiniMap->value > 0.0f)
			return true;
	}
