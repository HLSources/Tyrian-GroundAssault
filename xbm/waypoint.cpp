...
unsigned short *shortest_path[MAX_TEAMS] = {NULL, NULL, NULL, NULL};
unsigned short *from_to[MAX_TEAMS] = {NULL, NULL, NULL, NULL};
...
int WaypointFindPath(PATH **pPath, int *path_index, int waypoint_index, TEAM_ID team_id)
{
	int index;
	int count = 0;
	...
	while (*pPath != NULL)
	{
		while (*path_index < MAX_PATH_INDEX)
		{
			if ((*pPath)->index[*path_index] != -1)  // found a path?
			{
				...
				if ((team_id != TEAM_NONE) && (waypoints[index].flags & W_FL_TEAM_SPECIFIC) && ((waypoints[index].flags & W_FL_TEAM) != (team_id-1)))
				{
					(*path_index)++;
					continue;
				}
...
int WaypointFindNearest(edict_t *pEntity, float range, TEAM_ID team_id)
{
	if (num_waypoints < 1)
		return -1;
...
//	int team = team_id-1;
	...
	for (index=0; index < num_waypoints; index++)
	{
		...
		if (is_team_play && (team_id != TEAM_NONE) && (waypoints[index].flags & W_FL_TEAM_SPECIFIC) && ((waypoints[index].flags & W_FL_TEAM) != (team_id-1)))
			continue;
...
int WaypointFindNearest(Vector v_src, edict_t *pEntity, float range, TEAM_ID team_id)
{
	if (num_waypoints < 1)
		return -1;
...
int WaypointFindNearestGoal(edict_t *pEntity, int src, TEAM_ID team_id, int flags)
{
	if (num_waypoints < 1)
		return -1;
...
int WaypointFindNearestGoal(edict_t *pEntity, int src, TEAM_ID team_id, int flags, int exclude[])
{
	if (num_waypoints < 1)
		return -1;
...
int WaypointFindNearestGoal(Vector v_src, edict_t *pEntity, float range, TEAM_ID team_id, int flags)
{
	int index, min_index;
...
int WaypointFindRandomGoal(edict_t *pEntity, TEAM_ID team_id, int flags)
{
	if (num_waypoints < 1)
		return -1;
...
int WaypointFindRandomGoal(edict_t *pEntity, TEAM_ID team_id, int flags, int exclude[])
{
	if (num_waypoints < 1)
		return -1;
...
int WaypointFindRandomGoal(Vector v_src, edict_t *pEntity, float range, TEAM_ID team_id, int flags)
{
	if (num_waypoints < 1)
		return -1;
...
//	tfc_backpack_index = -1;	// "null" out backpack index
	...
	while ((pent = UTIL_FindEntityInSphere( pent, origin, radius )) != NULL)
	{
		...
		if (tr.flFraction >= 1.0)
		{
			...
			if (mod_id == TYRIAN_DLL && XDM_EntityIs)
			{
...
int WaypointFindReachable(edict_t *pEntity, float range, TEAM_ID team_id)
{
   ...
   for (matrix=0; matrix < MAX_TEAMS; matrix++)
   {
      if (build_matrix[matrix])
      {
         ...
         if (access(filename2, 0) == 0)  // does the .HPB_wpX file exist?
         {
            ...
            {
               sprintf(msg, "loading waypoint paths for team %d\n", matrix+1);
               ...
               {
                  ALERT(at_console, "Error reading waypoint paths!\n");
         ...
         if (shortest_path[matrix] == NULL)
         {
            sprintf(msg, "calculating waypoint paths for team %d...\n", matrix+1);
            ALERT(at_console, msg);
            ...
            {
               ALERT(at_console, "Error writing waypoint paths!\n");
            }

            sprintf(msg, "waypoint path calculations for team %d complete!\n",matrix+1);
            ALERT(at_console, msg);
         }
      }
   }
...
unsigned short WaypointRouteFromTo(int src, int dest, TEAM_ID team_id, bool shortest)
{
	unsigned short *pReturn;

	if (!IsValidTeam(team_id))
		return -1;

	if (team_id == TEAM_NONE)  // -1 means non-team play
		team_id = TEAM_1;// use team 1

	if (from_to[team_id-1] == NULL)  // if no team specific waypoints use team 0
		team_id = TEAM_1;// use team 1

	if (from_to[team_id-1] == NULL)  // if no route information just return
		return -1;

	if (shortest)
		pReturn = shortest_path[team_id-1];
	else
		pReturn = from_to[team_id-1];

	return pReturn[src * route_num_waypoints + dest];
}

/*
unsigned short WaypointDistanceFromTo(int src, int dest, TEAM_ID team_id)
{
	...
	return pShortestPath[src * route_num_waypoints + dest];
}*/
