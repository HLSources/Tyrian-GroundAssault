...
TYPEDESCRIPTION	gEntvarsDescription[] = 
{
	...
	DEFINE_ENTITY_FIELD( endpos, FIELD_VECTOR ),// XDM3035b
	...
	for (i = 0; i < ENTVARS_COUNT; ++i)
	{
		...
		if ( !stricmp( pField->fieldName, pkvd->szKeyName ) )
		{
			...
			case FIELD_SOUNDNAME:
				if (*pkvd->szValue == 0)
					ALERT(at_aiconsole, "Warning! Null string field in entity! %s:%s == \"%s\"\n", pkvd->szClassName, pkvd->szKeyName, pkvd->szValue);
			...
			case FIELD_VECTOR:
				{
					...
					{
						ALERT(at_console, "Bad vector field in entity! %s:%s == \"%s\"\n", pkvd->szClassName, pkvd->szKeyName, pkvd->szValue);
						VectorClear(vec);
					}
				}
				break;
...
void CSave :: WriteFunction( const char *pname, const int *data, int count )
{
	...
	if (functionName)
		...
		ALERT(at_error, "Invalid function pointer (%s) in entity!\n", pname);
}
