...
int CFlyingMonster :: CheckLocalMove ( const Vector &vecStart, const Vector &vecEnd, CBaseEntity *pTarget, float *pflDist )
{
	...
	if (FBitSet(pev->flags, FL_SWIM) && (POINT_CONTENTS(vecEnd) != CONTENTS_WATER))
	{
...
float CFlyingMonster :: ChangeYaw(float yawSpeed)
{
...
void CFlyingMonster::Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, int iGib)
{
...
void CFlyingMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case FLYING_AE_FLAPSOUND:
		if ( m_pFlapSound )
			EMIT_SOUND( edict(), CHAN_BODY, m_pFlapSound, GetSoundVolue(), ATTN_NORM );	
		break;
...
{
	int conPosition = POINT_CONTENTS(position);
	...
	int conProbe = POINT_CONTENTS(probe);
	...
	while (diff > 1.0)
	{
		...
		if (POINT_CONTENTS(position+midProbeVec) == conPosition)
		{
