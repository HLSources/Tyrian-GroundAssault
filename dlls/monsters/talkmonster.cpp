...
void CTalkMonster :: StartTask( Task_t *pTask )
{
	...
	case TASK_TLK_STOPSHOOTING:
		...
		PlaySentence( m_szGrp[TLK_NOSHOOT], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_NORM );
		TaskComplete();
		break;

	case TASK_CANT_FOLLOW:
		StopFollowing( FALSE );
		PlaySentence( m_szGrp[TLK_STOP], RANDOM_FLOAT(2, 2.5), GetSoundVolue(), ATTN_NORM );
		TaskComplete();
		break;
	...
	case TASK_MOVE_AWAY_PATH:
		{
			...
			ANGLE_VECTORS( dir, move, NULL, NULL );
...
//			UTIL_MakeVectors( pPlayer->v.angles );
			Vector forward;
			ANGLE_VECTORS(pPlayer->v.angles, forward, NULL, NULL);
			if ( UTIL_DotPoints( pPlayer->v.origin, pev->origin, forward ) < m_flFieldOfView )
			{
				// player looked away
				TaskFail();
			}
...
void CTalkMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_SENTENCE:				// Play a named sentence group
		ShutUpFriends();
		PlaySentence( pEvent->options, RANDOM_FLOAT(2.8, 3.4), GetSoundVolue(), ATTN_IDLE );
...
//	int pitch = GetVoicePitch();
	...
	// play response
	PlaySentence( m_szGrp[TLK_ANSWER], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
}
...
int CTalkMonster :: FIdleStare( void )
{
	...
	PlaySentence( m_szGrp[TLK_STARE], RANDOM_FLOAT(5, 7.5), GetSoundVolue(), ATTN_IDLE );
		...
		if (pPlayer)
		{
			if (FInViewCone(pPlayer) && FVisible(pPlayer))
			{
				...
				if (FBitSet(pev->spawnflags, SF_MONSTER_PREDISASTER))
					PlaySentence( m_szGrp[TLK_PHELLO], RANDOM_FLOAT(3, 3.5), GetSoundVolue(),  ATTN_IDLE );
				else
					PlaySentence( m_szGrp[TLK_HELLO], RANDOM_FLOAT(3, 3.5), GetSoundVolue(),  ATTN_IDLE );
		...
		if ( pTarget->IsPlayer() )
		{
			if ( pTarget->IsAlive() )
			{
					...
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->max_health / 8))
				{
					...
					PlaySentence( m_szGrp[TLK_PLHURT3], duration, GetSoundVolue(), ATTN_IDLE );
					...
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->max_health / 4))
				{
					...
					PlaySentence( m_szGrp[TLK_PLHURT2], duration, GetSoundVolue(), ATTN_IDLE );
					...
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->max_health / 2))
				{
					...
					PlaySentence( m_szGrp[TLK_PLHURT1], duration, GetSoundVolue(), ATTN_IDLE );
...
//				PlaySentence(m_szGrp[TLK_PLDEAD], duration, GetSoundVolue(), ATTN_IDLE);// XDM3035c
//				SetBits(m_bitsSaid, bit_saidHelloPlayer);// LOL :)
			}
		}
	}
	...
	{
		PlaySentence( szQuestionGroup, duration, GetSoundVolue(), ATTN_IDLE );
	...
	if ( RANDOM_LONG(0,1) )
	{
		...
		{
			m_hTalkTarget = pFriend;
			PlaySentence( szIdleGroup, duration, GetSoundVolue(), ATTN_IDLE );
			m_nSpeak++;
			return TRUE;
		}
	}
	...
	case SCHED_IDLE_STAND:
		{
			...
			if (FOkToSpeak())
			{
				if (!FBitSet(m_bitsSaid, bit_saidWoundLight) && (pev->health <= (pev->max_health * 0.75)))
				{
					...
					PlaySentence( m_szGrp[TLK_WOUND], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
				...
				else if (!FBitSet(m_bitsSaid, bit_saidWoundHeavy) && (pev->health <= (pev->max_health * 0.5)))
				{
					...
					//CTalkMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT(2.8, 3.2);
					PlaySentence( m_szGrp[TLK_MORTAL], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
...
//					UTIL_MakeVectors ( pPlayer->v.angles );
					...
					ANGLE_VECTORS(pPlayer->v.angles, forward, NULL, NULL);
					if ( ( pPlayer->v.origin - pev->origin ).Length2D() < TLK_STARE_DIST	&&
						 UTIL_DotPoints( pPlayer->v.origin, pev->origin, forward ) >= m_flFieldOfView )
					{
...
//		ALERT ( at_aiconsole, "Clear smell bits\n" );
	...
	{
		PlaySentence( m_szGrp[TLK_SMELL], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
...
void CTalkMonster::StopFollowing( BOOL clearSchedule )
{
	if ( IsFollowing() )
	{
		if ( !(m_afMemory & bits_MEMORY_PROVOKED) )
		{
			PlaySentence( m_szGrp[TLK_UNUSE], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
			m_hTalkTarget = m_hTargetEnt;
		}
...
void CTalkMonster::StartFollowing( CBaseEntity *pLeader )
{
	...
	PlaySentence( m_szGrp[TLK_USE], RANDOM_FLOAT(2.8, 3.2), GetSoundVolue(), ATTN_IDLE );
	m_hTalkTarget = m_hTargetEnt;
...
void CTalkMonster :: FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pCaller != NULL && pCaller->IsPlayer())
	{
		...
		if (IsLockedByMaster() || (pev->spawnflags & SF_MONSTER_PREDISASTER))
		{
			DeclineFollowing();
		}
		else if ( CanFollow() )
...
//		m_iszDecline = ALLOC_STRING(pkvd->szValue);
	...
	else if (FStrEq(pkvd->szKeyName, "SpeakAs"))// SHL
	{
