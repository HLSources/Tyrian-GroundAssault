...
/*static */int g_iRelationshipTable[16][16] =
{			 //   NONE	MACH	PLYR	HPASS	HMIL	AMIL	APASS	AMONST	APREY	APRED	INSECT	PLRALY	PBWPN	ABWPN	GREN	GIB
/*NONE*/		{ R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO },
/*MACHINE*/		{ R_NO,	R_NO,	R_DL,	R_DL,	R_NO,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_FR,	R_NO },
/*PLAYER*/		{ R_NO,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_AL,	R_FR,	R_FR,	R_FR,	R_NO },
/*HUMANPASSIVE*/{ R_NO,	R_NO,	R_AL,	R_AL,	R_HT,	R_FR,	R_NO,	R_HT,	R_DL,	R_FR,	R_NO,	R_AL,	R_NO,	R_NO,	R_FR,	R_NO },
/*HUMANMILITAR*/{ R_NO,	R_NO,	R_HT,	R_DL,	R_NO,	R_HT,	R_NO,	R_DL,	R_DL,	R_DL,	R_NO,	R_HT,	R_FR,	R_FR,	R_FR,	R_NO },
...
/*ALIENPREY   */{ R_NO,	R_FR,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO,	R_NO,	R_FR,	R_HT,	R_DL,	R_NO,	R_NO,	R_HT,	R_DL },// XDM3034 TEST crabs may be dumb enough to eat grenades =)
/*ALIENPREDATO*/{ R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO,	R_HT,	R_DL,	R_NO,	R_DL,	R_NO,	R_NO,	R_NO,	R_NO },
/*INSECT*/		{ R_NO,	R_NO,	R_FR,	R_FR,	R_FR,	R_FR,	R_NO,	R_FR,	R_FR,	R_FR,	R_NO,	R_FR,	R_NO,	R_NO,	R_NO,	R_HT },
/*PLAYERALLY*/	{ R_NO,	R_DL,	R_AL,	R_AL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO,	R_NO,	R_FR,	R_NO },
/*PBIOWEAPON*/	{ R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_NO,	R_DL,	R_NO,	R_NO },
/*ABIOWEAPON*/	{ R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_AL,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO },
/*GRENADE*/		{ R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_DL,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO },
/*GIB*/			{ R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO }
};
...
void CBaseMonster::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "class"))// XDM
	{
		m_iClass = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "m_iClass"))// stupid SHL names
	{
		m_iClass = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "mst_event_trigger"))
	{
		if (atoi(pkvd->szValue) > 0)
			m_bPlayMusicEvent = TRUE;
		else
			m_bPlayMusicEvent = FALSE;

		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "mst_event_track"))
	{
		m_iszMusicEventTrack = pkvd->szValue;
		pkvd->fHandled = TRUE;
	}
...
/*			if (pSightEnt == NULL)
			...
			if (pSightEnt != this && !FBitSet(pSightEnt->pev->spawnflags, SF_MONSTER_PRISONER) && pSightEnt->IsAlive())
			{
				...
				if (IRelationship(pSightEnt) != R_NO && !FBitSet(pSightEnt->pev->flags, FL_NOTARGET) && FInViewCone(pSightEnt) && FVisible(pSightEnt))
				{
/*					if (Classify() == CLASS_HUMAN_MILITARY || Classify() == CLASS_ALIEN_PREDATOR)
					{
						if (pSightEnt->pev->light_level < 32)// XDM: too dark
							continue;
					}*/
					if (pSightEnt->IsPlayer())
					{
						if (g_pGameRules->IsTeamplay())// XDM: team turrets, etc.
						{
							if (pev->team > 0 && pev->team == pSightEnt->pev->team)
								continue;
						}
...
{
	// right now  dmg = mysize/playersize * playerdamageformula
...
{
//	ALERT(at_console, "%s::CallMonsterThink() (%s)\n", STRING(pev->classname), STRING(pev->targetname));
	this->MonsterThink();
}
...
//	ALERT(at_console, "%s %d::MonsterThink() (%s)\n", STRING(pev->classname), entindex(), STRING(pev->targetname));
	...
	if (pev->waterlevel >= 3)// XDM
	{
		if (m_flBurnTime > gpGlobals->time)
		{
			m_flBurnTime = -1.0f;// stop burning
		}
	}
...
//	UTIL_ParticleEffect ( m_Route[ m_iRouteIndex ].vecLocation, g_vecZero, 255, 25 );
	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		...
		WRITE_BYTE( BLAST_SKIN_SHOCKWAVE ); // frame start
...
//		ASSERT(!HasConditions(bits_COND_SEE_ENEMY));
	...
	if ( HasConditions( bits_COND_SEE_ENEMY ) )
	{
		...
		if (!pEnemy->pev->velocity.IsZero())// != g_vecZero)
		{
			   ...
			   && (m_Route[m_iRouteIndex+1].iType & bits_MF_TO_NODE) == bits_MF_TO_NODE)
			{
				...
				if ( iLink >= 0 && WorldGraph.m_pLinkPool[iLink].m_pLinkEnt != NULL )
				{
					...
					if ( WorldGraph.HandleLinkEnt ( iSrcNode, WorldGraph.m_pLinkPool[iLink].m_pLinkEnt, m_afCapability, CGraph::NODEGRAPH_DYNAMIC ) )
					{
						...
						if (pevDoor)
						{
							m_flMoveWaitFinished = OpenDoorAndWait(CBaseEntity::Instance(pevDoor));
...
//		REMOVE_ENTITY(ENT(pev));
	...
	m_hEnemy			= NULL;
	m_flDistTooFar		= g_psv_zmax->value;
	m_flDistLook		= g_psv_zmax->value;// XDM
	m_flBurnTime		= 0.0f;
...
void CBaseMonster::StartMonster (void)
{
	...
	if ( pev->movetype != MOVETYPE_FLY && !FBitSet( pev->spawnflags, SF_MONSTER_FALL_TO_GROUND ) )
	{
		...
		{
			ALERT(at_console, "%s \"%s\" stuck in wall--level design error", STRING(pev->classname), STRING(pev->targetname));
...
BOOL CBaseMonster::FindCover(const Vector &vecThreat, const Vector &vecViewOffset, float flMinDist, float flMaxDist)
{
	...
	{
//SPAM	ALERT ( at_aiconsole, "Graph not ready for findcover!\n" );
		return FALSE;
	}
...
BOOL CBaseMonster::BuildNearestRoute(const Vector &vecThreat, const Vector &vecViewOffset, float flMinDist, float flMaxDist)
{
	...
	if ( !WorldGraph.m_fGraphPresent || !WorldGraph.m_fGraphPointersSet )
	{
//SPAM	ALERT ( at_aiconsole, "Graph not ready for BuildNearestRoute!\n" );
		return FALSE;
	}
...
{
	float flCurrentYaw = UTIL_AngleMod(pev->angles.y);// FOUND THE ERROR! pev->angles.y is somehow out of bounds!?
...
{
	GetEyePosition(GET_MODEL_PTR(ENT(pev)), pev->view_ofs);
...
void CBaseMonster::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_SOUND:			// Play a named wave file
		EMIT_SOUND_DYN(edict(), CHAN_BODY, pEvent->options, GetSoundVolue(), ATTN_IDLE, 0, m_voicePitch);
		break;

	case SCRIPT_EVENT_SOUND_VOICE:
		EMIT_SOUND_DYN(edict(), CHAN_VOICE, pEvent->options, GetSoundVolue(), ATTN_IDLE, 0, m_voicePitch);
		break;
	...
	case SCRIPT_EVENT_SENTENCE:			// Play a named sentence group
		SENTENCEG_PlayRndSz( edict(), pEvent->options, GetSoundVolue(), ATTN_IDLE, 0, m_voicePitch);
		break;
...
{
	if (WorldGraph.m_pNodes == NULL)// XDM3037
	{
//		ALERT(at_aiconsole, "CBaseMonster()::FGetNodeRoute() WorldGraph.m_pNodes == NULL!\n");
		return FALSE;
	}
...
//	m_iClass = CLASS_GIB;// TODO? affects game logic and frags
...
CBaseEntity *CBaseMonster::DropItem(const char *pszItemName, const Vector &vecPos, const Vector &vecAng)
{
	...
	CBaseEntity *pItem = CBaseEntity::Create(MAKE_STRING(pszItemName), vecPos, vecAng, pev->velocity, edict(), SF_NORESPAWN);
	if (pItem)
	{
//		pItem->pev->spawnflags |= SF_NORESPAWN;// XDM3035: dropped items should never respawn!
		// do we want this behavior to be default?!(sjb)
//		pItem->pev->velocity = pev->velocity;
		pItem->pev->avelocity = Vector(0.0f, RANDOM_FLOAT(0, 100), 0.0f);
/*		if (pItem->IsPlayerItem())
		{
			CBasePlayerItem *pPlayerItem = (CBasePlayerItem *)pItem;
...
void CBaseMonster::FrozenStart(float freezetime)
{
	...
	pev->rendercolor = Vector(63,143,255);
...
BOOL CBaseMonster::IsHuman(void)
{
	switch(Classify())
	{
	case CLASS_PLAYER:
...
public:
	...
	virtual int Classify(void) {return pev->frags;};
	virtual STATE GetState(void) { return pev->health?STATE_ON:STATE_OFF; };
};
...
{
	if (ShouldToggle(useType, GetState() != STATE_OFF))// XDM3037: TESTME: cast from STATE to bool
	{
		if (pev->health)
			pev->health = 0;
		else
