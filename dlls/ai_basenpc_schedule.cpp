...
BOOL CBaseMonster :: FScheduleValid ( void )
{
	...
	if ( HasConditions( m_pSchedule->iInterruptMask | bits_COND_SCHEDULE_DONE | bits_COND_TASK_FAILED ) )
	{
		...
		{
			// XDM3035b: SPAM, uncomment for testing
...
//	SPAM		ALERT ( at_aiconsole, "Schedule Failed at %d!\n", m_iScheduleIndex );
...
//					pev->solid = SOLID_NOT;
				...
				if (ShouldRespawn())// XDM3035: it was SO hard to figure out where to place this code!!
				{
					SetThink(&CBaseEntity::SUB_Respawn);
					pev->nextthink = gpGlobals->time + mp_monsrespawntime.value;
				}
				else
				{
	...
	case TASK_GET_PATH_TO_ENEMY:
		{
			...
			{
				// no way to get there =(
//				ALERT(at_aiconsole, "CBaseMonster(%d)::StartTask(%d) TASK_GET_PATH_TO_ENEMY failed!", entindex(), pTask->iTask);
				TaskFail();
			}
			break;
		}
	case TASK_GET_PATH_TO_ENEMY_CORPSE:
		{
			Vector fw;
			ANGLE_VECTORS(pev->angles, fw, NULL, NULL);// XDM3035a
			if ( BuildRoute ( m_vecEnemyLKP - fw * 64.0f, bits_MF_TO_LOCATION, NULL ) )
			{
				TaskComplete();
			}
			else
			{
//				ALERT(at_aiconsole, "CBaseMonster(%d)::StartTask(%d) TASK_GET_PATH_TO_ENEMY_CORPSE failed!", entindex(), pTask->iTask);
				TaskFail();
			}
		}
		break;
...
//				ALERT ( at_aiconsole, "GetPathToSpot failed!!\n" );
	...
	case TASK_GET_PATH_TO_HINTNODE:// for active idles!
		{
			if (WorldGraph.m_pNodes && MoveToLocation( m_movementActivity, 2, WorldGraph.m_pNodes[ m_iHintNode ].m_vecOrigin))// XDM3037
			{
				TaskComplete();
			}
			else
...
//				ALERT ( at_aiconsole, "GetPathToBestScent failed!!\n" );
	...
	default:
		{
			ALERT(at_aiconsole, "CBaseMonster(%d)::StartTask(%d) unknown task!\n", entindex(), pTask->iTask);
//			ALERT ( at_aiconsole, "No StartTask entry for %d\n", (SHARED_TASKS)pTask->iTask );
	...
	if (HasConditions(bits_COND_TASK_FAILED))
		ALERT(at_aiconsole, "CBaseMonster(%d)::StartTask(%d) task failed!\n", entindex(), pTask->iTask);
#endif
}
...
Schedule_t *CBaseMonster :: GetSchedule ( void )
{
	...
	case MONSTERSTATE_COMBAT:
		{
			if ( HasConditions( bits_COND_ENEMY_DEAD ) )
			{
				...
				{
					SetMonsterState( MONSTERSTATE_ALERT );
					return GetSchedule();
				}
			}
	...
	default:
		{
			ALERT(at_aiconsole, "CBaseMonster(%d %s)::GetSchedule(): invalid state %d!", entindex(), STRING(pev->classname), m_MonsterState);
			break;
		}
	}
