...
{
	studiohdr_t *pstudiohdr = (studiohdr_t *)pmodel;
	if (pstudiohdr == NULL || pev->sequence >= pstudiohdr->numseq )
		return 0;
...
float SetBlending(void *pmodel, entvars_t *pev, byte iBlender, float flValue)
{
	...
	short setting = 255 * (flValue - pseqdesc->blendstart[iBlender]) / (pseqdesc->blendend[iBlender] - pseqdesc->blendstart[iBlender]);
...
int FindTransition( void *pmodel, int iEndingAnim, int iGoalAnim, int *piDir )
{
	...
	ALERT(at_console, "Error in transition graph %s\n", pstudiohdr->name);
	return iGoalAnim;
}
...
void SetBodygroup(void *pmodel, entvars_t *pev, int iGroup, int iValue)
{
	...
	if (pstudiohdr == NULL)
	{
		ALERT(at_console, "ERROR: Bad studio header in SetBodygroup!\n");
		return;
	}
...
int GetBodygroup(void *pmodel, entvars_t *pev, int iGroup)
{
	...
	if (pstudiohdr == NULL)
	{
		ALERT(at_console, "ERROR: Bad studio header in GetBodygroup!\n");
		return 0;
	}
...
int GetBodyCount(void *pmodel, int bodygroup)
{
	...
	if (pstudiohdr == NULL)
	{
		ALERT(at_console, "ERROR: Bad studio header in GetBodyCount!\n");
		return 0;
	}
...
void SetBones(void *pmodel, float (*data)[3], int datasize)
{
	...
	if (pstudiohdr == NULL)
	{
		ALERT(at_console, "ERROR: Bad studio header in SetBones!\n");
		return;
	}
