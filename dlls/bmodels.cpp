...
void SetMovedir(entvars_t *pev)
{
	...
	{
		ANGLE_VECTORS(pev->angles, pev->movedir, NULL, NULL);// XDM3035: TESTME
	}
...
void CFuncWall::Spawn(void)
{
	...
	pev->nextthink = 0;// XDM: reduce lag? gpGlobals->time + 2.0;
...
/*	if (FStringNull(pev->targetname))
	{
		ALERT(at_aiconsole, "%s with no targetname, making static.\n", STRING(pev->classname));
...
//	void EXPORT SloshTouch( CBaseEntity *pOther );
	...
	virtual int	ObjectCaps(void) { return CBaseEntity::ObjectCaps() & ~FCAP_ACROSS_TRANSITION; }
	virtual BOOL IsBSPModel(void) { return TRUE; }// XDM3034: test!
};
...
//	pev->flags |= FL_WORLDBRUSH;// XDM3034: test!
...
//	if (g_pGameRules->IsMultiplayer())// TEST
//		MAKE_STATIC(ENT(pev));
}
...
//	else
	...
	if (fvol > 0.0f)// XDM3035c: FIXED: POSSIBLE!
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRINGV(pev->noiseRunning), fvol, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
	}
}
...
void CFuncRotating::SpinUp(void)
{
		...
		abs(vecAVel.z) >= abs(pev->movedir.z * pev->speed))
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRINGV(pev->noiseRunning), m_flVolume, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL, FANPITCHMAX);
...
void CFuncRotating::SpinDown(void)
{
		...
		((vecdir < 0) && (vecAVel.x >= 0 && vecAVel.y >= 0 && vecAVel.z >= 0)))
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRINGV(pev->noiseRunning /* Stop */), 0, 0, SND_STOP, m_pitch);
		SetThink(&CFuncRotating::Rotate);
		Rotate();
	}
	else
...
void CFuncRotating::RotatingUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( FBitSet ( pev->spawnflags, SF_BRUSH_ACCDCC ) )
	{
		...
		if ( pev->avelocity != g_vecZero )
		{
			...
			//EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, STRINGV(pev->noiseStop),
		...
		{
			SetThink(&CFuncRotating::SpinUp);
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRINGV(pev->noiseRunning), 0.01, m_flAttenuation, 0, FANPITCHMIN);
			pev->nextthink = pev->ltime + 0.1;
		}
	}
	else if ( !FBitSet ( pev->spawnflags, SF_BRUSH_ACCDCC ) )//this is a normal start/stop brush.
	{
		...
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRINGV(pev->noiseRunning), m_flVolume, m_flAttenuation, 0, FANPITCHMAX);
...
void CPendulum::RopeTouch ( CBaseEntity *pOther )
{
	if ( !pOther->IsPlayer() )
	{// not a player!
// XDM		ALERT ( at_console, "Not a client\n" );
		return;
	}
