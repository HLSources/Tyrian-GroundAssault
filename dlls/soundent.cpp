...
void CSoundEnt :: InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration )
{
	...
	if (!g_pGameRules->FAllowMonsters())
		return;// XDM: only monsters need this

	int iThisSound = pSoundEnt->IAllocSound();

	if (iThisSound == SOUNDLIST_EMPTY)
	{
		ALERT(at_aiconsole, "Could not AllocSound() for InsertSound()\n");
		return;
	}
...
void CSoundEnt :: Initialize ( void )
{
	...
	for ( i = 0 ; i < gpGlobals->maxClients ; ++i)
	{
		...
		{
			ALERT(at_console, "DLL: Could not AllocSound() for Client Reserve!\n");
			return;
		}
...
CSound *CSoundEnt::SoundPointerForIndex(const int &iIndex)
{
	...
	if ( iIndex < 0 )
	{
		ALERT(at_aiconsole, "SoundPointerForIndex() - Index < 0!\n");
		return NULL;
	}
