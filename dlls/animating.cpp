...
{
// this 'Spawn' function is used as inline... mostly.
//	Precache();
//	CBaseDelay::Spawn();
...
{
	ASSERT( activity != 0 );
	return ::LookupActivity(GET_MODEL_PTR(ENT(pev)), pev, activity);
}
...
{
	return ::LookupActivityHeaviest(GET_MODEL_PTR(ENT(pev)), pev, activity);
}
...
{
	return ::LookupSequence(GET_MODEL_PTR(ENT(pev)), label);
}
...
{
	return ::GetSequenceFlags(GET_MODEL_PTR(ENT(pev)), pev);
}
...
{
	return SetController(GET_MODEL_PTR(ENT(pev)), pev, iController, flValue);
}
...
{
	return ::SetBlending(GET_MODEL_PTR(ENT(pev)), pev, iBlender, flValue);
}
