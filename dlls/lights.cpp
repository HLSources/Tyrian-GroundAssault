...
void CLight :: KeyValue( KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "_cone"))// XDM
		pkvd->fHandled = TRUE;
	else if (FStrEq(pkvd->szKeyName, "_cone2"))// XDM
		pkvd->fHandled = TRUE;
	else if (FStrEq(pkvd->szKeyName, "_fade"))// XDM
		pkvd->fHandled = TRUE;
	else if (FStrEq(pkvd->szKeyName, "_sky"))// XDM
		pkvd->fHandled = TRUE;
	else
...
//		CHANGE_METHOD(ENT(pev), em_use, light_use);
		if (FBitSet(pev->spawnflags, SF_LIGHT_START_OFF))
			...
			LIGHT_STYLE(m_iStyle, STRINGV( m_iszPattern ));
		else
