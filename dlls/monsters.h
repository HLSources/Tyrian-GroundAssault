...
#define SF_MONSTER_INVULNERABLE			1024
...
#define R_FR	-1// (FEAR) will run away
...
//	virtual void OnFreePrivateData(void);// XDM3035c
	...
	static void SpawnRandomGibs(CBaseEntity *pVictim, int cGibs, int human, Vector velocity);
	static void SpawnStickyGibs(CBaseEntity *pVictim, const Vector &vecOrigin, int cGibs);
	static int SpawnModelGibs(CBaseEntity *pVictim, const Vector &origin, const Vector &mins, const Vector &maxs, const Vector &velocity, int rndVel, int modelIndex, int count, float life, int material, int bloodcolor, int flags = 0);// XDM
...
extern int g_iRelationshipTable[16][16];
